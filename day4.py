...
with open('input/day4.txt', "r") as f:
    ...
    infile = [s.strip() for s in f.read().split('\n\n')]
    base_boards = [s.split('\n') for s in infile]
    base_boards = [[s.split() for s in line] for line in base_boards]

# add a 'stamped' flag
boards = [[[(x, False) for x in s] for s in line] for line in base_boards]
